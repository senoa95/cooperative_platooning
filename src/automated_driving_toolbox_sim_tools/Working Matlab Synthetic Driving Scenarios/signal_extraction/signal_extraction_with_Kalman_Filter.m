...
%         currFollowerVal__leftLaneDist = follower.laneStats.leftLane.distance(i);
%         currFollowerVal__rightLaneDist = follower.laneStats.rightLane.distance(i);
%         currLeaderVal_leftLaneDist = leader.laneStats.leftLane.distance(i);
%         currLeaderVal_rightLaneDist = leader.laneStats.rightLane.distance(i);
%         mahalD_leftLaneDist(i) = ((currFollowerVal__leftLaneDist - currLeaderVal_leftLaneDist)^2/(followerVar_leftLaneDist + leaderVar_leftLaneDist))^0.5;
%         mahalD_rightLaneDist(i) = ((currFollowerVal__rightLaneDist - currLeaderVal_rightLaneDist )^2/(followerVar_rightLaneDist + leaderVar_rightLaneDist))^0.5;
%         if mahalD_leftLaneDist(i) > mahalD_rightLaneDist(i) && testPose(i) < distanceThresh
%             usableDist_rightLaneDist(i) = follower.laneStats.rightLane.distance(i);
%             usableDist_leftLaneDist(i) = sent.laneStats.laneWidth(i) + usableDist_rightLaneDist(i);
%             corrected = corrected + 1;
%         elseif mahalD_leftLaneDist(i) < mahalD_rightLaneDist(i) && testPose(i) < distanceThresh
%             usableDist_leftLaneDist(i) = follower.laneStats.leftLane.distance(i);
%             usableDist_rightLaneDist(i) = -(sent.laneStats.laneWidth(i) - usableDist_leftLaneDist(i));
%             corrected = corrected + 1;
%         elseif mahalD_leftLaneDist(i) == mahalD_leftLaneDist(i)
%             usableDist_rightLaneDist(i) = follower.laneStats.rightLane.distance(i);
%             usableDist_leftLaneDist(i) = follower.laneStats.leftLane.distance(i);
%         end
%         usableDist_rightLaneDist(i) = follower.laneStats.rightLane.distance(i);
...
Sigma_min = zeros(1,numel(unifiedStruct.timeVec)); % covariance after Propagation
Sigma_plus = zeros(1,numel(unifiedStruct.timeVec)); % covariance after update
...
for i = 1:numel(follower.time)
    ...
    if testPose(i) < distanceThresh
        sigmaRelHeading = testPose(i);
        [rightLaneHeadingAngle_followerEst(i), Sigma_min]  =...
            yawEstimate_propagate(sent.laneStats.rightLane.sentHeadingAngle(i), Sigma_plus, relativeHeading(i), sigmaRelHeading);
        [leftLaneHeadingAngle_followerEst(i), Sigma_min]  =...
            yawEstimate_propagate(sent.laneStats.leftLane.sentHeadingAngle(i), Sigma_plus, relativeHeading(i), sigmaRelHeading);
...
function [x_hat_min, Sigma_min]  = distEstimate_propagate(x_hat_plus, Sigma_plus, u, sigma_u, dt)
...
%                     sqrt((currHeadwayLeaderValue(augmentIdx,1) - currHeadwayFollowerValue(augmentIdx,1))^2 + (currHeadwayLeaderValue(augmentIdx,2) - currHeadwayFollowerValue(augmentIdx,2))^2);
...
%                             currHeadwayLeaderValue(i,:) =  allLeaderStats.(currLane).(currStatName)(1,i);
%                         end
