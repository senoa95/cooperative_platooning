...
%%
syms y1 x1 y2 x2 rho rhoDot
eqns  = [
    (rho*x1^2)*(1/2) + (rhoDot * x1^3)*(1/6) - y1 == 0, ...
    (rho*x2^2)*(1/2) + (rhoDot * x2^3)*(1/6) - y2 == 0];
vars = [rho, rhoDot];
rhoSolved = solve(eqns, [rho rhoDot]);
rho = rhoSolved.rho;
rhoDot = rhoSolved.rhoDot;
...
%%
function comPuteRealCurvature()
for wpIdx = 1:size(waypoints,1) - 1
    rhoSolved = [];
    currX = waypoints(wpIdx,1);
    nextX = waypoints(wpIdx + 1,1);
    currY = waypoints(wpIdx + 1, 2);
    nextY = waypoints(wpIdx, 2);
    x1 = currX;
    y1 = currY;
    x2 = nextX;
    y2 = nextY;
    
    rhoVal(wpIdx) = subs(rho);
    rhoDotVal(wpIdx) = subs(rhoDot);
end
end
%%
